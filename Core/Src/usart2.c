...
bool default_packet_parcer(char *buffer, uint32_t size);

bool (*packet_parcer)(char *, uint32_t) = default_packet_parcer;
...
void DMA1_Stream5_IRQHandler(void)
{
	if (READ_BIT(DMA1->HISR, DMA_HISR_TCIF5))
	{
		...
		if (packet_parcer(&husart2.rx_buffer[last_end_point], BUFFER_SIZE - last_end_point)) {
			UART2_Packet_Recieved_Callback();
		}
...
void USART2_IRQHandler(void)
{
	if (READ_BIT(USART2->SR, USART_SR_IDLE))
	{
		...
		if (packet_parcer(&husart2.rx_buffer[last_end_point], packet_size)) {
			UART2_Packet_Recieved_Callback();
		}
...
__weak void UART2_Packet_Recieved_Callback() {
